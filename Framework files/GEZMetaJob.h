...
/**
 ...
 - @"standardInput" key (GEZTaskSubmissionStandardInputKey) : NSString or NSData for the standard input.
	The string or the data is used as the standard input for the taks, with one exception: if it is a string and the string is an absolute path to an existing file, the meta job will use instead the contents of that file.
